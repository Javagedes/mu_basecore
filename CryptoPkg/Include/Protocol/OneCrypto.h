...
/**
  Build the ordered certificate chain from a PKCS#7 signer up to a
  caller-supplied trust anchor.

  Starting from SignerCert, this function walks issuer links using the
  certificates embedded in the PKCS#7 SignedData blob (AuthData). Each
  hop is cryptographically verified with X509VerifyCert() until it
  reaches TrustAnchorCert. The result is an EFI_CERT_STACK ordered from
  the signer at index 0 to the trust anchor at index N-1.

  The returned chain is the cryptographically-verified chain (the certs
  that actually issued one another), safe to use for per-certificate
  revocation checks (e.g. dbx). The anchor MUST come from a trusted
  store (e.g. db).
  ...
  @param[in]   AuthData             Pointer to the PKCS#7 SignedData
                                    blob (DER-encoded).
  @param[in]   AuthDataSize         Size of AuthData in bytes.
  @param[in]   SignerCert           DER-encoded signer certificate that
                                    starts the chain.
  @param[in]   SignerCertSize       Size of SignerCert in bytes.
  @param[in]   TrustAnchorCert      DER-encoded trust anchor certificate
                                    that ends the chain.
  @param[in]   TrustAnchorCertSize  Size of TrustAnchorCert in bytes.
  @param[out]  CertChain            Receives a newly allocated
                                    EFI_CERT_STACK buffer ordered
                                    signer..anchor. Caller frees with
                                    FreePool().
  @param[out]  CertChainSize        Receives the CertChain length.

  @retval EFI_SUCCESS            The chain was built and returned.
  @retval EFI_NOT_FOUND          Could not link the signer up to the
                                 trust anchor via AuthData.
  @retval EFI_INVALID_PARAMETER  Bad parameter or malformed input.
  @retval EFI_OUT_OF_RESOURCES   Memory allocation failed.
  @retval EFI_UNSUPPORTED        Interface not supported.
  ...
  @ingroup PKCS
**/
typedef EFI_STATUS (EFIAPI *ONE_CRYPTO_PKCS7_GET_CERTIFICATE_CHAIN)(
  IN  CONST UINT8  *AuthData,
  IN  UINTN        AuthDataSize,
