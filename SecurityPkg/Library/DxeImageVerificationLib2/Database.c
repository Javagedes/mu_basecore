...
/**
  Load the platform's db and dbx signature databases and compute the
  set of image-hash algorithms currently enrolled across them.
  ...
  @param[out]  HashAlgorithms  Set of image-hash signature-type GUIDs
                               currently enrolled in db or dbx.

  @retval EFI_SUCCESS            Databases loaded and HashAlgorithms
                                 populated. *Db / *Dbx may still be NULL
                                 if the corresponding variable was
                                 absent.
  @retval EFI_INVALID_PARAMETER  A required pointer is NULL.
  @retval EFI_VOLUME_CORRUPTED   One of the database buffers is
