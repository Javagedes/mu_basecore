...
/**
  ...
  The subject is identified by Cache->Type:
    * DigestCacheTypeImage - the image is matched by its digest under each list's hash algorithm.
    * DigestCacheTypeX509  - the certificate is matched either by exact DER bytes against an
                             EFI_CERT_X509_GUID list, or by its TBSCertificate digest against a
                             cert-hash list.

  The walk is confined to the valid prefix produced by the iterators. If a malformed list, an
  entry region too small for the digest, or a hash-computation failure prevents the walk from being
  proven complete, *Truncated is set so a deny-list caller can fail closed; an allow-list caller
  ignores it.
  ...
  )
{
  ...
  //
  // An unusable subject cache cannot be searched for. Report it as an incomplete walk: a deny-list
  // caller (IsInDbx) then fails closed, while an allow-list caller (IsInDb) simply finds no match.
  //
  ...
  )
{
  ...
  //
  // Until proven otherwise the certificate is unusable, and no authority is available.
  //
  ...
  //
  // Prelude: parse the PKCS#7 and determine the image-hash algorithm. A parse failure leaves the
  // certificate "unusable" - a verdict about the certificate, not an error.
  //
  ...
  //
  // Record the hash algorithm now that it is known; it applies to whatever verdict we reach.
  //
  CopyGuid (&Evaluation->Authority.SignatureType, &HashType);

  //
  // Absent an authorizing anchor the verdict is "not in db".
  //
