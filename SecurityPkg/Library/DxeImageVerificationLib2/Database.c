...
{
  // TODO: implement signer-TBS-hash lookup against EFI_CERT_X509_<HASH>_GUID lists.
  (VOID)List;
  (VOID)AuthData;
  (VOID)AuthDataSize;
  (VOID)ImageHash;
...
/**
  ...
  @param[in]      Databases  The `db` / `dbx` signature databases to evaluate against.
  @param[out]     Authority  On authorization, Authority->Data references the EFI_SIGNATURE_DATA
                             trust anchor in `db` that authorized the image and Authority->Size is
                             its SignatureSize. On no authorization, Authority->Data is NULL and
                             Authority->Size is 0.

  @retval EFI_SUCCESS            The evaluation completed; inspect Authority for the result.
  @retval EFI_INVALID_PARAMETER  A required pointer is NULL.
  @retval other                  A failure (PKCS#7 extraction, hash-algorithm lookup, or
                                 image-hash computation) prevented evaluation.
**/
EFI_STATUS
  ...
  )
{
  ...
  //
  // IsCertAuthorized populates Authority only when it authorizes the image; on a non-authorizing
  // result it leaves the NULL/0 values set above.
  //
  IsCertAuthorized (
    AuthData,
    AuthDataSize,
