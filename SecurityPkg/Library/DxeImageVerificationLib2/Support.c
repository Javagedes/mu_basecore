...
/**
  ...
  @retval RETURN_SUCCESS            Directory processed successfully, or ignored.
  @retval RETURN_INVALID_PARAMETER  CallbackContext or DataDirectory is NULL for
  ...
  )
{
  ...
  if ((CallbackContext == NULL) || (DataDirectory == NULL)) {
    return RETURN_INVALID_PARAMETER;
  }
