/** @file
  ...
  All iterators use the same Init / Next contract. Init validates the
  entire container up front and returns an EFI_STATUS. After a
  successful Init, Next is infallible: it returns the next item or
  NULL once iteration is complete. No Status field is exposed; if
  Init succeeded, the container is well-formed end-to-end.
...
/**
  ...
  Fields are owned by the iterator implementation; callers should
...
/**
  Iterator state for walking EFI_SIGNATURE_DATA entries inside a
  single EFI_SIGNATURE_LIST.
**/
typedef struct {
...
/**
  Iterator state for walking WIN_CERTIFICATE records inside a PE/COFF
  image's security data directory.
**/
typedef struct {
  CONST UINT8    *Cursor;     ///< Next byte to consume.
...
/**
  ...
  Init validates every list header in the buffer end-to-end. After a
  successful return, DatabaseIterNext is infallible.
  ...
  @retval EFI_INVALID_PARAMETER  Iter is NULL, or Buffer is NULL with a
                                 non-zero BufferSize.
  @retval EFI_VOLUME_CORRUPTED   The buffer contains a malformed
                                 EFI_SIGNATURE_LIST (bad size fields,
                                 trailing bytes, or payload not a
                                 multiple of SignatureSize).
**/
EFI_STATUS
DatabaseIterInit (
...
/**
  ...
  Init validates that the directory described by SecDataDir lies
  entirely within the supplied file buffer and walks every
  WIN_CERTIFICATE header to verify its dwLength fits the remaining
  table. After a successful return, WinCertIterNext is infallible.
...
/**
  ...
  Callers that want to skip unsupported certificate types should
  inspect the returned record's wCertificateType field; the iterator
