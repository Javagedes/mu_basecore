/** @file
  ...
  LoadSignatureDatabase and LoadSignatureDatabases are exercised
  against a mocked GetVariable2.
...
// ---------------------------------------------------------------------------
// LoadSignatureDatabases (db + dbx + hash-algorithm survey)
// ---------------------------------------------------------------------------
...
TEST_F (LoadSignatureDatabasesTest, NullDb_ReturnsInvalidParameter) {
  ...
  EXPECT_EQ (
    LoadSignatureDatabases (NULL, &DbSize, &Dbx, &DbxSize, &HashAlgorithms),
    EFI_INVALID_PARAMETER
    );
}
...
TEST_F (LoadSignatureDatabasesTest, NullDbSize_ReturnsInvalidParameter) {
  ...
  EXPECT_EQ (
    LoadSignatureDatabases (&Db, NULL, &Dbx, &DbxSize, &HashAlgorithms),
    EFI_INVALID_PARAMETER
    );
}
...
TEST_F (LoadSignatureDatabasesTest, NullDbx_ReturnsInvalidParameter) {
  ...
  EXPECT_EQ (
    LoadSignatureDatabases (&Db, &DbSize, NULL, &DbxSize, &HashAlgorithms),
    EFI_INVALID_PARAMETER
    );
}
...
TEST_F (LoadSignatureDatabasesTest, NullDbxSize_ReturnsInvalidParameter) {
  ...
  EXPECT_EQ (
    LoadSignatureDatabases (&Db, &DbSize, &Dbx, NULL, &HashAlgorithms),
    EFI_INVALID_PARAMETER
    );
}

TEST_F (LoadSignatureDatabasesTest, NullHashAlgorithms_ReturnsInvalidParameter) {
  VOID   *Db = NULL;
  UINTN  DbSize;
  VOID   *Dbx = NULL;
  UINTN  DbxSize;
  ...
  EXPECT_EQ (
    LoadSignatureDatabases (&Db, &DbSize, &Dbx, &DbxSize, NULL),
    EFI_INVALID_PARAMETER
    );
}

TEST_F (LoadSignatureDatabasesTest, BothVariablesMissing_SuccessEmptyHashSet) {
  EXPECT_CALL (UefiLibMock, GetVariable2 (_, _, _, _))
  ...
  EXPECT_EQ (
    LoadSignatureDatabases (&Db, &DbSize, &Dbx, &DbxSize, &HashAlgorithms),
    EFI_SUCCESS
    );
  ...
  EXPECT_EQ (HashAlgorithms.Count, 0u);
}

TEST_F (LoadSignatureDatabasesTest, OnlyDbPresent_DbAllocatedAndHashSurveyed) {
  ...
  EXPECT_EQ (Dbx, (VOID *)NULL);
  EXPECT_EQ (DbxSize, 0u);
  ASSERT_EQ (HashAlgorithms.Count, 1u);
  EXPECT_EQ (CompareGuid (&HashAlgorithms.Guids[0], &gEfiCertSha256Guid), TRUE);

  FreePool (Db);
}

TEST_F (LoadSignatureDatabasesTest, OnlyDbxPresent_DbxAllocatedAndHashSurveyed) {
  ...
  ASSERT_EQ (HashAlgorithms.Count, 1u);
  EXPECT_EQ (CompareGuid (&HashAlgorithms.Guids[0], &gEfiCertSha384Guid), TRUE);

  FreePool (Dbx);
}

TEST_F (LoadSignatureDatabasesTest, BothPresent_BuffersAllocatedAndHashUnion) {
  std::vector<UINT8>  DbBuf;
  std::vector<UINT8>  DbxBuf;
  ...
  ASSERT_NE (Dbx, (VOID *)NULL);
  EXPECT_EQ (DbxSize, DbxBuf.size ());
  ASSERT_EQ (HashAlgorithms.Count, 2u);
  EXPECT_EQ (CompareGuid (&HashAlgorithms.Guids[0], &gEfiCertSha256Guid), TRUE);
  EXPECT_EQ (CompareGuid (&HashAlgorithms.Guids[1], &gEfiCertSha384Guid), TRUE);
...
TEST_F (LoadSignatureDatabasesTest, MalformedDb_CorruptedReturnedAndPartialFreed) {
  // A db variable that is structurally invalid (SignatureListSize
  // smaller than the EFI_SIGNATURE_LIST header) causes
  // GetDatabaseHashAlgorithms to fail with EFI_VOLUME_CORRUPTED.
  // Both partial allocations must be released and the out-pointers
  // nulled.
  std::vector<UINT8>  DbBuf (sizeof (EFI_SIGNATURE_LIST), 0);

  ((EFI_SIGNATURE_LIST *)DbBuf.data ())->SignatureListSize   = 1;  // bogus
  ((EFI_SIGNATURE_LIST *)DbBuf.data ())->SignatureHeaderSize = 0;
  ((EFI_SIGNATURE_LIST *)DbBuf.data ())->SignatureSize       = sizeof (EFI_GUID);
