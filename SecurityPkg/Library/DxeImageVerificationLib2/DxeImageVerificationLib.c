  ...
  )
{
  ...
  // A failure to search `dbx` is treated as forbidden (fail closed).
  //
  ...
  //
  // If the image is unsigned, the iterator is empty and this step is a no-op.
  //
  ...
  )
{
  ...
  //
  return ValidateImage (FileBuffer, FileSize, &SecDataDir, GetMeasuredAuthorities ());
}
