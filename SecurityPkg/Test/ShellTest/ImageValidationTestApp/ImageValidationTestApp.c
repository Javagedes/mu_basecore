  ...
  )
{
  ...
  // _MULTI_SIGNED) then hash algorithm (SHA-256 / 384 / 512). Corrupt image variants are
  ...
  //
  CONST UINT8  *DigestData[IMAGE_TYPE_MULTI_SIGNED + 1][3] = {
    { mUnsignedImageDigestSha256,    mUnsignedImageDigestSha384,    mUnsignedImageDigestSha512    },
    { mSignedImageDigestSha256,      mSignedImageDigestSha384,      mSignedImageDigestSha512      },
    { mMultiSignedImageDigestSha256, mMultiSignedImageDigestSha384, mMultiSignedImageDigestSha512 }
  };
  CONST UINTN  DigestDataSize[IMAGE_TYPE_MULTI_SIGNED + 1][3] = {
    { mUnsignedImageDigestSha256Size,    mUnsignedImageDigestSha384Size,    mUnsignedImageDigestSha512Size    },
    { mSignedImageDigestSha256Size,      mSignedImageDigestSha384Size,      mSignedImageDigestSha512Size      },
    { mMultiSignedImageDigestSha256Size, mMultiSignedImageDigestSha384Size, mMultiSignedImageDigestSha512Size }
  };
  CONST struct {
    UINT32            Flag;
    UINTN             HashIndex;
  ...
  )
{
  ...
  for (ScenarioIndex = 0; ScenarioIndex < mScenarioCount; ScenarioIndex++) {
    ...
    CHAR8        TestDescription[300];
    CONST CHAR8  *TestId;
    EFI_STATUS   GenerateStatus;

    //
    // Auto-generate the human-readable scenario description from DB state, DBX state, and
    // expected status.
    //
    ...
    //
    // Prefix the description with the scenario's full ID so a failing test reported on the
    // console points straight back to its README entry. The ID is also used as the test
    // case name in the machine-readable (xUnit) results.
    //
    TestId = mScenarios[ScenarioIndex].Id;
    AsciiSPrint (TestDescription, sizeof (TestDescription), "%a: %a", TestId, ScenarioDescription);
